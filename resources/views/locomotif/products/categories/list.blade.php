	...
	<div class='listing-elements-hold'>
		@foreach($productsCategories as $key => $category)
			<div class='listing-element {{ $loop->last ? 'lastElement' : '' }}'>
				...
				<div class='listing-box'>
					<p><a href='/admin/productsCategories/{{$category->id}}/edit'>{{ $category->category_name }}</a></p>
				</div>
                <div class='listing-box flex06 alignCenter'>
					<p>{{ $category->category_url }}</p>
				</div>
				<div class='listing-box flex03 alignCenter'>
					<p>{{\Carbon\Carbon::parse($category->created_at)->format('Y-d-m')}}</p>
				</div>
                
				<div class='listing-box flex04 alignCenter'>
					<span class='general-btn noPointer status-{{$category->category_status}}'>{{$category->category_status}}</span>
				</div>
				<div class='listing-box flex02 alignCenter'>
					<div class='more-actions-tab'>
						...
						<ul class='more-list'>
							<li>
								<form action="/admin/productsCategories/{{ $category->id }}" method="POST">@method('DELETE') @csrf <input type="submit" value="Șterge"/></form>
							</li>
						</ul>
					</div>
				</div>
