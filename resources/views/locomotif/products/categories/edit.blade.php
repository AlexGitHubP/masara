...
<div class='details-bar'><!--details-bar-->
	<div class='details-left'>
		<h2>Editează: {{ \Illuminate\Support\Str::limit($productCategory->category_name, $limit = 15, $end = '...') }} </h2>
	</div>
	...
	<div class='details-right'>
        <ul class='action-controls'>
			<li>
				<a class='general-btn backBtn' href="/admin/productsCategories">Înapoi</a>
			</li>
			<li>
				<a href="/admin/productsCategories/create" class='iconControl'>
					<span>
    ...
    <div class="cms-body">
        <div class="tab-content">
			<div class="tab-pane active" id="detalii">
                <form method="POST" action='/admin/productsCategories/{{$productCategory->id}}'>
                    @csrf
                    @method('PUT')
                    ...
                    <div class='flex-inputs flex100'>
                        <div class="input-element">
                            ...
                            <input class='builNiceUrl' data-target='category_url' type="text" name="category_name" id='category_name' value="{{$productCategory->category_name}}">
                        </div>
                        <div class="input-element">
                            <label for="category_url">Url categorie</label>
                            <input type="text" name="category_url" id='category_url' value="{{$productCategory->category_url}}">
                        </div>
                        <div class="input-element textarea">
                            <label for="category_description">Descriere categorie</label>
                            <textarea name="category_description" id="category_description" value='{{$productCategory->category_description}}'>{{$productCategory->category_description}}</textarea>
                        </div>
                        <div class="input-element">
                            <label for="category_status">Status</label>
                            <select name='category_status' id='category_status' value="{{$productCategory->category_status}}">
                                <option value="">Alege status</option>
                                <option value="published" {{$productCategory->category_status=='published' ? 'selected' : '' }}>Publicat</option>
								<option value="hidden"    {{$productCategory->category_status=='hidden' ? 'selected' : '' }}>Ascuns</option>
                            </select>
                        </div>
