...
<div class='details-list'><!--details-list-->
	<div class='details-left'>
		<h2>Listă Subcategorii produse</h2>
	</div>
...
<div class="content-container">
	...
	<div class='listing-elements-hold'>
		@foreach($productsSubcategories as $key => $subcategory)
			<div class='listing-element {{ $loop->last ? 'lastElement' : '' }}'>
				<div class='listing-box flex02 alignCenter'>
					<p>{{$key+1}}</p>
				</div>
				<div class='listing-box'>
					<p><a href='/admin/productsSubcategories/{{$subcategory->id}}/edit'>{{ $subcategory->subcategory_name }}</a></p>
				</div>
				<div class='listing-box alignCenter'>
					<p><a href='/admin/productsCategories/{{$subcategory->category_id}}/edit'>{{ $subcategory->category->category_name }}</a></p>
				</div>
                <div class='listing-box flex06 alignCenter' >
					<p>{{ $subcategory->subcategory_url }}</p>
				</div>
				<div class='listing-box flex03 alignCenter'>
					<p>{{\Carbon\Carbon::parse($subcategory->created_at)->format('Y-d-m')}}</p>
				</div>
                
				<div class='listing-box flex04 alignCenter'>
					<span class='general-btn noPointer status-{{$subcategory->subcategory_status}}'>{{$subcategory->subcategory_status}}</span>
				</div>
				<div class='listing-box flex02 alignCenter'>
					<div class='more-actions-tab'>
						...
						<ul class='more-list'>
							<li>
								<form action="/admin/productsSubcategories/{{ $subcategory->id }}" method="POST">@method('DELETE') @csrf <input type="submit" value="Șterge"/></form>
							</li>
						</ul>
					</div>
				</div>
