...
@section('title', 'Lista zone')

@section('content')
<div class='details-list'><!--details-list-->
	<div class='details-left'>
		<h2>Listă zone produse</h2>
	</div>
...
<div class='filter-tab'>
	...
	<div class='actions-tab'>
		<ul class='action-controls'>
			<li>
				<a href="/admin/productsArea/create" class='iconControl'>
					<span>
	...
	<div class='listing-elements-hold'>
		@foreach($list as $key => $item)
			<div class='listing-element {{ $loop->last ? 'lastElement' : '' }}'>
				...
				<div class='listing-box'>
					<p><a href='/admin/productsArea/{{$item->id}}/edit'>{{ $item->area_name }}</a></p>
				</div>
                <div class='listing-box flex06 alignCenter'>
					<p>{{ $item->area_url }}</p>
				</div>
				...
				<div class='listing-box flex02 alignCenter'>
					<div class='more-actions-tab'>
						...
						<ul class='more-list'>
							<li>
								<form action="/admin/productsArea/{{ $item->id }}" method="POST">@method('DELETE') @csrf <input type="submit" value="Șterge"/></form>
							</li>
						</ul>
					</div>
				</div>
