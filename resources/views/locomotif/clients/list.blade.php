...
<div class='filter-tab'>
	...
	<div class='actions-tab'>
		<ul class='action-controls'>
			<li>
				<a href="/admin/clients/create" class='iconControl'>
					<span>
	...
	<div class='listing-elements-hold'>
		@foreach($items as $key => $item)
			<div class='listing-element {{ $loop->last ? 'lastElement' : '' }}'>
				...
				<div class='listing-box flex04'>
					<p><a href='/admin/clients/{{$item->id}}/edit'>{{ $item->name }} {{ $item->surname }}</a></p>
				</div>
                <div class='listing-box flex03 alignCenter'>
					<p>{{ $item->email }}</p>
				</div>
				...
				<div class='listing-box flex02 alignCenter'>
					<div class='more-actions-tab'>
						...
						<ul class='more-list'>
							<li>
								<form action="/admin/clients/{{ $item->id }}" method="POST">@method('DELETE') @csrf <input type="submit" value="Șterge"/></form>
							</li>
						</ul>
					</div>
				</div>
