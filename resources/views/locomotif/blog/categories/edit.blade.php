    ...
    <div class="cms-body">
        <div class="tab-content">
			<div class="tab-pane active" id="detalii">
                <form method="POST" action='/admin/blogCategories/{{$item->id}}'>
                    @csrf
                    @method('PUT')
                    ...
                    <div class='flex-inputs flex100'>
                        <div class="input-element">
                            ...
                            <input class='builNiceUrl' data-target='category_url' type="text" name="category_name" id='category_name' value="{{$item->category_name}}">
                        </div>
                        <div class="input-element">
                            <label for="category_url">Url categorie</label>
                            <input type="text" name="category_url" id='category_url' value="{{$item->category_url}}">
                        </div>
                        <div class="input-element textarea">
                            <label for="category_description">Descriere categorie</label>
                            <textarea class='tinymce' name="category_description" id="category_description" value='{{$item->category_description}}'>{{$item->category_description}}</textarea>
                        </div>
                        <div class="input-element">
