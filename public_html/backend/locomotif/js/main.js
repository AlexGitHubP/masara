...
function initTinyMce(){
    var owner_id = document.querySelector('textarea.tinymce').dataset.ownerid;
    ...
    tinymce.init({
        ...
        images_upload_url: '/admin/media/uploadMceImages?owner_id='+owner_id,
        ...
        paste_preprocess: function(plugin, args) {
            ...
            var cleanedContent = stripStylesAndClasses(content);
            // Update the processed content
            args.content = cleanedContent;
          }
    });

    function stripStylesAndClasses(content) {
        ...
        var comments = tempDiv.querySelectorAll('comment');
        comments.forEach(function(comment) {
          if (comment.textContent.trim() === '[if !supportLists]') {
    ...
    $('input[type="radio"][name="main_subcategory"]').change(function() {
        if ($(this).is(':checked')) {
          ...
          $.ajax({
            ...
            showSnackbar('error', 'A intervenit o eroare. Te rog încearcă din nou.', 4000)
        }
    });

    $('input[type="radio"][name="main_category"]').change(function() {
        if ($(this).is(':checked')) {
          // The radio input is checked
          ...
          $.ajax({
                ...
                cache: false,
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                url:'/admin/productsCategories/setMain',
            }).done(function(response){
                showSnackbar(response.type, response.message, 2000)
            }).fail(function(response){
                showSnackbar(response.type, response.message, 4000)
